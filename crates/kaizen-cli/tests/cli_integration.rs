//! Integration tests for kaizen CLI commands.
//!
//! Runs the actual CLI binary as a subprocess and verifies output.
//! Requires HelixDB running at localhost:6969 (`just db`).
//!
//! Run: `cargo test -p kaizen-cli --no-default-features --test cli_integration -- --ignored`
...
fn run_cli(args: &[&str]) -> (String, String, i32) {
    let output = Command::new(cli_bin())
        .args(args)
        .output()
        .expect("failed to run kaizen CLI");
...
fn test_cli_status() {
    ...
    assert_eq!(code, 0, "kaizen status should exit 0");
    assert!(
        stdout.contains("Kaizen Status"),
...
fn test_cli_search() {
    ...
    assert_eq!(code, 0, "kaizen search should exit 0");
...
fn test_cli_search_json_output() {
    ...
    assert_eq!(code, 0, "kaizen search --json should exit 0");
...
fn test_cli_search_with_kind_filter() {
    ...
    assert_eq!(code, 0, "kaizen search --kind fact should exit 0");
}
...
fn test_cli_export_import_roundtrip() {
    ...
    let export_path = format!("/tmp/kaizen-test-export-{}.json", uuid::Uuid::now_v7());

    // Export
    let (stdout, _stderr, code) = run_cli(&["export", "-o", &export_path]);
    assert_eq!(code, 0, "kaizen export should exit 0");
...
fn test_cli_prune_dry_run() {
    ...
    assert_eq!(code, 0, "kaizen prune --dry-run should exit 0");
...
fn test_cli_reembed_dry_run() {
    ...
    assert_eq!(code, 0, "kaizen reembed --dry-run should exit 0");
    assert!(
...
fn test_cli_init_creates_config() {
    let test_dir = format!("/tmp/kaizen-init-test-{}", uuid::Uuid::now_v7());
    ...
    let output = Command::new(cli_bin())
        ...
        .output()
        .expect("failed to run kaizen init");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let code = output.status.code().unwrap_or(-1);

    assert_eq!(code, 0, "kaizen init should exit 0");
    assert!(
        stdout.contains("Created") || stdout.contains("config"),
        "should confirm config creation: {stdout}"
    );

    // Verify .kaizen/config.toml exists
    let config_path = format!("{}/.kaizen/config.toml", test_dir);
    assert!(
