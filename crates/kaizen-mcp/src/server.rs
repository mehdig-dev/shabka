...
use kaizen_core::assess::{self, AssessConfig, IssueCounts};
use kaizen_core::config::{self, EmbeddingState, KaizenConfig};
use kaizen_core::dedup::{self, DedupDecision};
...
#[derive(Clone)]
pub struct KaizenServer {
    storage: Arc<HelixStorage>,
    embedder: Arc<EmbeddingService>,
    config: Arc<KaizenConfig>,
    user_id: String,
...
fn to_mcp_error(e: KaizenError) -> ErrorData {
    match &e {
        KaizenError::NotFound(_) => ErrorData::resource_not_found(
            e.to_string(),
            Some(serde_json::json!({"error_type": "not_found"})),
        ),
        KaizenError::InvalidInput(_) => ErrorData::invalid_params(
            e.to_string(),
            Some(serde_json::json!({"error_type": "invalid_input"})),
        ),
        KaizenError::Config(_) => ErrorData::invalid_params(
            e.to_string(),
            Some(serde_json::json!({"error_type": "config_error"})),
        ),
        _ => {
            let variant = match &e {
                KaizenError::Storage(_) => "storage_error",
                KaizenError::Helix(_) => "helix_error",
                KaizenError::Http(_) => "http_error",
                KaizenError::Embedding(_) => "embedding_error",
                KaizenError::Serialization(_) => "serialization_error",
                _ => "internal_error",
            };
...
impl KaizenServer {
    ...
    async fn save_memory(
        ...
        kaizen_core::model::validate_create_input(
            &params.title,
            &params.content,
            params.importance,
        )
        ...
        if user_tags_empty {
            if let Some(ref llm) = self.llm {
                if let Some(result) = kaizen_core::auto_tag::auto_tag(&memory, llm.as_ref()).await {
    ...
    async fn update_memory(
        ...
        kaizen_core::model::validate_update_input(&input).map_err(to_mcp_error)?;
        ...
        // Log history event
        let changes = kaizen_core::history::diff_update(&old_memory, &input);
        self.history.log(
    ...
    async fn history(
        ...
        let events: Vec<&kaizen_core::history::MemoryEvent> =
...
#[tool_handler]
impl ServerHandler for KaizenServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some(
                "Kaizen is a shared LLM memory system. Use progressive disclosure for efficient retrieval:\n\n\
...
mod tests {
    ...
    fn test_invalid_input_maps_to_invalid_params() {
        let err = KaizenError::InvalidInput("title cannot be empty".into());
        let data = to_mcp_error(err);
    ...
    fn test_config_maps_to_invalid_params() {
        let err = KaizenError::Config("bad config".into());
        let data = to_mcp_error(err);
        assert_eq!(data.code.0, -32602); // INVALID_PARAMS
        let error_type = data.data.unwrap()["error_type"]
            .as_str()
    ...
    fn test_storage_maps_to_internal_error() {
        let err = KaizenError::Storage("db failed".into());
        let data = to_mcp_error(err);
    ...
    fn test_embedding_maps_to_internal_error() {
        let err = KaizenError::Embedding("embed failed".into());
        let data = to_mcp_error(err);
        assert_eq!(data.code.0, -32603); // INTERNAL_ERROR
        let error_type = data.data.unwrap()["error_type"]
            .as_str()
