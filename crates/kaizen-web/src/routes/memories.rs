...
use kaizen_core::history::{EventAction, MemoryEvent};
use kaizen_core::model::*;
use kaizen_core::storage::StorageBackend;
use serde::Deserialize;
use uuid::Uuid;

use kaizen_core::config::EmbeddingState;

use crate::error::AppError;
use crate::AppState;

/// Returns the stale threshold from config (or default 90).
fn stale_days(config: &kaizen_core::config::KaizenConfig) -> i64 {
...
async fn list_memories(
    ...
    // Filter by privacy
    entries.retain(|e| kaizen_core::sharing::is_visible(e.privacy, &e.created_by, &state.user_id));
...
async fn create_memory(
    ...
    kaizen_core::model::validate_create_input(&input.title, &input.content, input.importance)?;

    let privacy = kaizen_core::sharing::parse_default_privacy(&state.config.privacy);
...
async fn update_memory(
    ...
    let changes = kaizen_core::history::diff_update(&old_memory, &update);
    state.history.log(
