...
impl HelixStorage {
    ...
    async fn query_once<T: Serialize, R: DeserializeOwned>(
        ...
        if !status.is_success() {
            return Err(KaizenError::Storage(format!(
...
fn record_to_memory(r: &MemoryRecord) -> Result<Memory> {
    ...
    Ok(Memory {
        id: Uuid::parse_str(&r.memory_id).map_err(|e| KaizenError::Storage(e.to_string()))?,
        kind: r.kind.parse().map_err(KaizenError::Storage)?,
        title: r.title.clone(),
        ...
        created_at: DateTime::parse_from_rfc3339(&r.created_at)
            ...
            .map_err(|e| KaizenError::Storage(e.to_string()))?,
...
fn record_to_session(r: &SessionRecord) -> Result<Session> {
    ...
    Ok(Session {
        id: Uuid::parse_str(&r.session_id).map_err(|e| KaizenError::Storage(e.to_string()))?,
        project_id: r.project_id.clone(),
