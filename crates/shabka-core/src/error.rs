...
pub type Result<T> = std::result::Result<T, ShabkaError>;
...
mod tests {
    ...
    fn test_transient_429() {
        let err = ShabkaError::Embedding("API error 429: rate limit exceeded".into());
        assert!(err.is_transient());
    }

    #[test]
    fn test_transient_503() {
        let err = ShabkaError::Embedding("API error 503: service unavailable".into());
        assert!(err.is_transient());
    }

    #[test]
    fn test_transient_timeout() {
        let err = ShabkaError::Embedding("connection timed out".into());
        assert!(err.is_transient());
    }

    #[test]
    fn test_transient_500() {
        let err = ShabkaError::Storage("HTTP 500 internal server error".into());
        assert!(err.is_transient());
    }

    #[test]
    fn test_permanent_401() {
        let err = ShabkaError::Embedding("API error 401: unauthorized".into());
        assert!(!err.is_transient());
    }

    #[test]
    fn test_permanent_config() {
        let err = ShabkaError::Config("missing API key".into());
        assert!(!err.is_transient());
    }
