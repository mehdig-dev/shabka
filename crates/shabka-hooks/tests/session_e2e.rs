//! End-to-end tests for session compression via the shabka-hooks binary.
//!
...
//!
//! Run: `cargo test -p shabka-hooks --no-default-features --test session_e2e -- --ignored`
...
fn send_hook_event(json: &str) -> (String, String, i32) {
    let mut child = Command::new(hooks_bin())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to spawn shabka-hooks");
