...
use clap::Parser;
use kaizen_core::assess::{self, AssessConfig, AssessmentResult, IssueCounts};
use kaizen_core::config::{
    self, EmbeddingConfig, EmbeddingState, GraphConfig, KaizenConfig, VALID_PROVIDERS,
};
use kaizen_core::decay::{self, PruneConfig, PruneResult};
use kaizen_core::embedding::EmbeddingService;
use kaizen_core::graph;
use kaizen_core::history::{EventAction, HistoryLogger, MemoryEvent};
use kaizen_core::model::*;
use kaizen_core::ranking::{self, RankCandidate, RankingWeights};
use kaizen_core::sharing;
use kaizen_core::storage::{HelixStorage, StorageBackend};
use owo_colors::OwoColorize;
use uuid::Uuid;

#[derive(Parser)]
#[command(name = "kaizen", about = "Kaizen: Shared LLM Memory System")]
enum Cli {
    /// Initialize Kaizen in the current project
    Init {
    ...
    Export {
        /// Output file path
        #[arg(short, long, default_value = "kaizen-export.json")]
        output: String,
    ...
    /// Run diagnostic checks on the Kaizen pipeline
    Doctor,
...
async fn main() -> Result<()> {
    ...
    let cli = Cli::parse();
    let config = KaizenConfig::load(Some(&std::env::current_dir()?))
        .unwrap_or_else(|_| KaizenConfig::default_config());
...
async fn run(cli: Cli, config: &KaizenConfig, user_id: &str) -> Result<()> {
    match cli {
...
fn make_storage(config: &KaizenConfig) -> HelixStorage {
    HelixStorage::new(
...
fn format_helix_error(err: &anyhow::Error, config: &KaizenConfig) -> String {
    let msg = format!("{:#}", err);
...
async fn cmd_init(provider: &str, check_only: bool) -> Result<()> {
    ...
    let kaizen_dir = cwd.join(".kaizen");

    if kaizen_dir.exists() {
        println!("Kaizen already initialized in this project.");
        return Ok(());
    }

    std::fs::create_dir_all(&kaizen_dir)?;

    let mut config = KaizenConfig::default_config();
    ...
    std::fs::write(kaizen_dir.join("config.toml"), toml_str)?;

    // Add config.local.toml to .gitignore if not already present
    let gitignore_path = cwd.join(".gitignore");
    let entry = ".kaizen/config.local.toml";
    ...
    println!("{}", "Initialized Kaizen in .kaizen/".green());
    println!("  {}   .kaizen/config.toml", "Config:".dimmed());
    println!("  {} {}", "Provider:".dimmed(), provider.cyan());
    println!(
        "  {}",
        "Edit .kaizen/config.local.toml for local overrides (gitignored)".dimmed()
    );
    println!();
...
async fn cmd_status(storage: &HelixStorage, config: &KaizenConfig, user_id: &str) -> Result<()> {
    println!("{}", "Kaizen Status".bold());
    ...
    let config_path = dirs::config_dir()
        .map(|p| p.join("kaizen").join("config.toml"))
...
async fn cmd_export(
    ...
    output: &str,
    privacy: &str,
    scrub_config: Option<&kaizen_core::scrub::ScrubConfig>,
    ...
    if scrub_report_only {
        if let Some(cfg) = scrub_config {
            ...
            for m in &memories {
                let text = format!("{}\n{}", m.title, m.content);
                let report = kaizen_core::scrub::analyze(&text, cfg);
    ...
    if let Some(cfg) = scrub_config {
        ...
        for m in &mut memories {
            ...
            m.title = kaizen_core::scrub::scrub(&m.title, cfg);
            m.content = kaizen_core::scrub::scrub(&m.content, cfg);
...
async fn cmd_assess(
    ...
    if counts.generic_titles > 0 {
        suggestions.push("Generic titles: review with `kaizen get <id>` and update titles");
    }
    if counts.no_tags > 0 {
        suggestions.push("No tags: add tags to improve searchability");
    }
    if counts.stale > 0 {
        suggestions.push("Stale: run `kaizen prune` to archive inactive memories");
    }
...
async fn cmd_doctor(config: &KaizenConfig) -> Result<()> {
    println!("{}", "Kaizen Doctor".bold());
    println!("{}", "=============".dimmed());
    println!();
    ...
    match EmbeddingService::from_config(&config.embedding) {
        Ok(service) => {
            // Try a test embed
            match service.embed("kaizen doctor test").await {
                Ok(vec) => {
                    println!(
                        "  {} Embedding      {} / {} ({}d, vec_len={})",
                        "OK".green(),
    ...
    // 4. Hooks binary
    match which::which("kaizen-hooks") {
        Ok(path) => {
            println!(
                "  {} Hooks binary   {}",
                "OK".green(),
        ...
        Err(_) => {
            println!(
                "  {} Hooks binary   {}",
                "FAIL".red(),
                "kaizen-hooks not found in PATH".red()
            );
            println!(
                "       {} Install with: {}",
                "hint:".dimmed(),
    ...
    if critical_fail {
        println!(
            "{}",
            "Some checks failed. Fix the issues above and re-run `kaizen doctor`.".red()
        );
        std::process::exit(1);
    } else if !helix_ok {
        println!(
...
async fn cmd_consolidate(
    ...
    let llm = kaizen_core::llm::LlmService::from_config(&config.llm)
    ...
    let result = kaizen_core::consolidate::consolidate(
        storage,
        embedder,
        &llm,
        &consolidate_config,
