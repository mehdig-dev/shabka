...
pub fn validate_create_input(title: &str, content: &str, importance: f32) -> Result<()> {
    ...
    if trimmed.is_empty() {
        return Err(KaizenError::InvalidInput("title cannot be empty".into()));
    }
    if trimmed.len() > MAX_TITLE_LENGTH {
        return Err(KaizenError::InvalidInput(format!(
...
/// The core entity in Kaizen. Represents a unit of captured knowledge.
