...
/// Uses the directory basename, e.g. "/home/user/projects/kaizen" → "kaizen".
...
/// Entry point for the kaizen-hooks binary.
///
...
fn run() -> anyhow::Result<()> {
    ...
    let config = KaizenConfig::load(Some(cwd)).unwrap_or_else(|_| KaizenConfig::default_config());
...
fn handle_stop(event: &HookEvent, config: &KaizenConfig) -> anyhow::Result<()> {
    let buffer = SessionBuffer::new(&event.session_id);
...
async fn compress_events(events: &[BufferedEvent], config: &KaizenConfig) -> Vec<CompressedMemory> {
    // Try LLM compression if enabled
    if config.llm.enabled {
        match kaizen_core::llm::LlmService::from_config(&config.llm) {
...
async fn save_compressed_memories(
    ...
    let llm_service = if config.llm.enabled && config.graph.dedup_llm {
        kaizen_core::llm::LlmService::from_config(&config.llm).ok()
    } else {
        None
    };
    ...
    for compressed in memories {
        ...
        if let Some(ref llm) = auto_tag_llm {
            if let Some(result) = kaizen_core::auto_tag::auto_tag(&memory, llm).await {
        ...
        // Dedup check
        let dedup_decision = kaizen_core::dedup::check_duplicate(
            &storage,
            &embedding,
            &config.graph,
            None,
        ...
        match dedup_decision {
            kaizen_core::dedup::DedupDecision::Skip {
                existing_title,
                similarity,
                ..
            } => {
            ...
            kaizen_core::dedup::DedupDecision::Supersede {
                existing_id,
                existing_title,
                similarity,
            } => {
                ...
                let _ = storage
                    .update_memory(
                        existing_id,
                        &kaizen_core::model::UpdateMemoryInput {
                            status: Some(kaizen_core::model::MemoryStatus::Superseded),
                            ..Default::default()
                        },
                    )
                    .await;
                let _ = storage
                    .add_relation(&kaizen_core::model::MemoryRelation {
                        source_id: memory.id,
                        target_id: existing_id,
                        relation_type: kaizen_core::model::RelationType::Supersedes,
                        strength: similarity,
                    })
                    .await;
            }
            kaizen_core::dedup::DedupDecision::Update {
                existing_id,
                existing_title,
                merged_content,
                merged_title,
            ...
            kaizen_core::dedup::DedupDecision::Contradict {
                existing_id,
                existing_title,
                similarity,
                reason,
                ...
                let _ = storage
                    .add_relation(&kaizen_core::model::MemoryRelation {
                        source_id: memory.id,
                        target_id: existing_id,
                        relation_type: kaizen_core::model::RelationType::Contradicts,
                        strength: similarity,
                    })
                    .await;
                kaizen_core::graph::semantic_auto_relate(
                    &storage, memory.id, &embedding, None, None,
                )
                .await;
                continue;
            }
            kaizen_core::dedup::DedupDecision::Add => {}
        }
        ...
        kaizen_core::graph::semantic_auto_relate(&storage, memory.id, &embedding, None, None).await;
    }
...
fn save_memory_immediate(
    event: &HookEvent,
    config: &KaizenConfig,
    kind: kaizen_core::model::MemoryKind,
    title: String,
    content: String,
    importance: f32,
    tags: Vec<String>,
    ...
    rt.block_on(async {
        ...
        if config.capture.auto_tag && config.llm.enabled {
            if let Ok(llm) = kaizen_core::llm::LlmService::from_config(&config.llm) {
                if let Some(result) = kaizen_core::auto_tag::auto_tag(&memory, &llm).await {
                    let mut tags = memory.tags.clone();
                    for tag in result.tags {
                        if !tags.contains(&tag) {
