...
mod tests {
    ...
    async fn test_non_transient_error_no_retry() {
        ...
        let result = with_retry(3, 1, || {
            attempts.fetch_add(1, Ordering::SeqCst);
            async { Err::<i32, _>(ShabkaError::Config("bad config".into())) }
        })
        .await;
    ...
    async fn test_transient_error_retries_up_to_max() {
        ...
        let result = with_retry(2, 1, || {
            attempts.fetch_add(1, Ordering::SeqCst);
            async { Err::<i32, _>(ShabkaError::Storage("HTTP 503 unavailable".into())) }
        })
        .await;
        assert!(result.is_err());
    ...
    async fn test_transient_error_succeeds_on_retry() {
        ...
        let result = with_retry(3, 1, || {
            ...
            async move {
                if attempt < 2 {
                    Err::<i32, _>(ShabkaError::Storage("HTTP 503 unavailable".into()))
                } else {
                    Ok(99)
                }
            }
