...
impl KaizenConfig {
    ...
    /// 1. ~/.config/kaizen/config.toml (global)
    /// 2. .kaizen/config.toml (project)
    /// 3. .kaizen/config.local.toml (local, gitignored)
    pub fn load(project_dir: Option<&Path>) -> Result<Self> {
        ...
        if let Some(dir) = project_dir {
            let project_config = dir.join(".kaizen").join("config.toml");
            if project_config.exists() {
                builder = builder.add_source(File::from(project_config).required(false));
            }

            // Layer 3: Local config (gitignored)
            let local_config = dir.join(".kaizen").join("config.local.toml");
        ...
        let config = builder
            .build()
            .map_err(|e| KaizenError::Config(e.to_string()))?;
...
fn global_config_path() -> Option<PathBuf> {
    dirs::config_dir().map(|p| p.join("kaizen").join("config.toml"))
}
...
/// to detect provider changes that require `kaizen reembed`.
...
pub struct EmbeddingState {
    ...
    /// RFC3339 timestamp of the last successful `kaizen reembed` run.
...
impl EmbeddingState {
    /// Path to the state file: `~/.config/kaizen/embedding_state.toml`
    pub fn path() -> Option<PathBuf> {
        dirs::config_dir().map(|p| p.join("kaizen").join("embedding_state.toml"))
    }
    ...
    pub fn save(&self) -> Result<()> {
        let path = Self::path()
            .ok_or_else(|| KaizenError::Config("cannot determine config directory".to_string()))?;
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| KaizenError::Config(format!("failed to create config dir: {e}")))?;
        }
        let toml_str = toml::to_string_pretty(self).map_err(|e| {
            KaizenError::Config(format!("failed to serialize embedding state: {e}"))
        })?;
        std::fs::write(&path, toml_str)
            .map_err(|e| KaizenError::Config(format!("failed to write embedding state: {e}")))?;
        Ok(())
    }
    ...
    pub fn migration_warning(config: &EmbeddingConfig, dimensions: usize) -> Option<String> {
        ...
        Some(format!(
            "WARNING: Embedding provider changed!\n\
             ...
             \x20 Run `kaizen reembed` to re-embed all memories with the new provider.",
            state.provider,
            state.model,
            state.dimensions,
            config.provider,
    ...
    if current_dims != state.dimensions {
        Err(format!(
            "Dimension mismatch: stored embeddings are {}d ({}/{}), \
             ...
             Run `kaizen reembed` to re-embed all memories.",
            state.dimensions,
            state.provider,
            state.model,
            current_dims,
...
mod tests {
    ...
    fn test_default_config() {
        let config = KaizenConfig::default_config();
    ...
    fn test_load_config_no_files() {
        ...
        let config = KaizenConfig::load(Some(Path::new("/nonexistent/path"))).unwrap();
        assert_eq!(config.helix.port, 6969);
        assert_eq!(config.web.port, 37737);
    }
    ...
    fn test_config_serde_roundtrip() {
        ...
        let parsed: KaizenConfig = toml::from_str(&toml_str).unwrap();
...
"#;
        let config: KaizenConfig = toml::from_str(toml_str).unwrap();
...
"#;
    ...
    fn test_embedding_state_roundtrip() {
        let dir = std::env::temp_dir().join(format!("kaizen-test-{}", std::process::id()));
