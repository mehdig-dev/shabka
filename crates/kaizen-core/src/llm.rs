...
impl LlmService {
    ...
    pub fn from_config(config: &LlmConfig) -> Result<Self> {
        let provider = match config.provider.as_str() {
            ...
            other => {
                return Err(KaizenError::Config(format!(
    ...
    async fn generate_ollama(&self, prompt: &str, system: Option<&str>) -> Result<String> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| KaizenError::Embedding(format!("Ollama LLM request failed: {e}")))?;
        ...
        let json: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| KaizenError::Embedding(format!("Ollama LLM response parse error: {e}")))?;

        json["response"]
            .as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| {
                KaizenError::Embedding("Ollama LLM response missing 'response' field".into())
            })
    }
    ...
    async fn generate_openai(&self, prompt: &str, system: Option<&str>) -> Result<String> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| KaizenError::Embedding(format!("OpenAI LLM request failed: {e}")))?;
        ...
        let json: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| KaizenError::Embedding(format!("OpenAI LLM response parse error: {e}")))?;

        json["choices"][0]["message"]["content"]
            .as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| KaizenError::Embedding("OpenAI LLM response missing content".into()))
    }
    ...
    async fn generate_anthropic(&self, prompt: &str, system: Option<&str>) -> Result<String> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| KaizenError::Embedding(format!("Anthropic LLM request failed: {e}")))?;
        ...
        let json: serde_json::Value = resp.json().await.map_err(|e| {
            KaizenError::Embedding(format!("Anthropic LLM response parse error: {e}"))
        })?;
        ...
        json["content"][0]["text"]
            ...
            .map(|s| s.to_string())
            .ok_or_else(|| {
                KaizenError::Embedding("Anthropic LLM response missing text content".into())
            })
    }
    ...
    async fn generate_gemini(&self, prompt: &str, system: Option<&str>) -> Result<String> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| KaizenError::Embedding(format!("Gemini LLM request failed: {e}")))?;
        ...
        let json: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| KaizenError::Embedding(format!("Gemini LLM response parse error: {e}")))?;

        json["candidates"][0]["content"]["parts"][0]["text"]
            .as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| KaizenError::Embedding("Gemini LLM response missing text".into()))
    }
}
